...
class LoadDataset(Dataset):
    ...
    def __init__(self, class_1_dir="...", class_2_dir="...", image_size=224):
        """
        ...
        self.transform = T.Compose(
            [
                ...
                T.ToTensor(),
                T.Normalize(mean=_MEAN_RGB, std=_STD_RGB),
            ]
        )
    ...
    def __init__(self, pretrained_name, out_channels=1, image_size=224):
        ...
        if pretrained_name in ["vit_small_patch14_reg4_dinov2.lvd142m"]:
            self.backbone = timm.create_model(
                pretrained_name, pretrained=True, img_size=image_size
            )
        else:
...
class Runner:
    def __init__(
        ...
        self.TRAIN_CONFIG.NUM_WORKERS = (
            ...
            if self.TRAIN_CONFIG.NUM_WORKERS > self.TRAIN_CONFIG.BATCH_SIZE
            else self.TRAIN_CONFIG.NUM_WORKERS
        )
        ...
        self.VALID_CONFIG.NUM_WORKERS = (
            self.VALID_CONFIG.BATCH_SIZE
            if self.VALID_CONFIG.NUM_WORKERS > self.VALID_CONFIG.BATCH_SIZE
            else self.VALID_CONFIG.NUM_WORKERS
        )

        self.TRAIN_VALID_RATIO = (
            train_valid_ratio if sum(train_valid_ratio) == 1 else [0.9, 0.1]
        )
    ...
    def create_dataloader(self):
        ...
        train_data, valid_data = torch.utils.data.random_split(
            dataset, self.TRAIN_VALID_RATIO
        )
    ...
    def train_step(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            num_iter += 1
            batch_train_accuracy = 100.0 * train_correct / train_total
            self.writer.add_scalar(
                "Iter/train_accuracy", batch_train_accuracy, num_iter
            )
            self.writer.add_scalar("Iter/train_loss", loss.item(), num_iter)
            self.writer.flush()
            ...
            if batch_idx % (batch_number * print_fre // 100) == 0:
                ...
                eta = (batch_time_end - batch_time_start) * (
                    remain_batch + batch_number * (self.TRAIN_CONFIG.EPOCH - epoch)
                )
                eta = str(datetime.timedelta(seconds=eta))
                iter_num = batch_idx * len(data)
                iter_num = str(iter_num).zfill(len(str(total_data)))
                total_percent = 100.0 * batch_idx / len(self.train_loader)

                print(
                    f'Train Epoch {epoch}: [{iter_num}/{total_data} ({total_percent:2.0f}%)] | Loss: {loss.item():.10f} | Acc: {batch_train_accuracy:.4f} | LR: {self.optimizer.param_groups[0]["lr"]:.10f} | ETA: {eta}'
                )

        train_accuracy = 100.0 * train_correct / len(self.train_loader.dataset)
    ...
    def valid_step(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.valid_loader):
            ...
            num_iter += 1
            batch_valid_accuracy = 100.0 * valid_correct / valid_total
            self.writer.add_scalar(
                "Iter/valid_accuracy", batch_valid_accuracy, num_iter
            )
            self.writer.add_scalar("Iter/valid_loss", loss.item(), num_iter)
            self.writer.flush()

        valid_accuracy = 100.0 * valid_correct / len(self.valid_loader.dataset)
    ...
    def train(self):
        print(
            f"[WARNING] TRAIN CONFIG: num_workers > batch_size ({self.TRAIN_CONFIG.NUM_WORKERS} > {self.TRAIN_CONFIG.BATCH_SIZE}) -> set num_workers to {self.TRAIN_CONFIG.BATCH_SIZE}"
        ) if self.TRAIN_CONFIG.NUM_WORKERS > self.TRAIN_CONFIG.BATCH_SIZE else None
        print(
            f"[WARNING] VALID CONFIG: num_workers > batch_size ({self.VALID_CONFIG.NUM_WORKERS} > {self.VALID_CONFIG.BATCH_SIZE}) -> set num_workers to {self.VALID_CONFIG.BATCH_SIZE}"
        ) if self.VALID_CONFIG.NUM_WORKERS > self.VALID_CONFIG.BATCH_SIZE else None
        print("[INFO] *** TRAIN CONFIG ***")
        print(f"[INFO] - Epoch: {self.TRAIN_CONFIG.EPOCH}")
        print(f"[INFO] - Batch size: {self.TRAIN_CONFIG.BATCH_SIZE}")
        print(f"[INFO] - Num workers: {self.TRAIN_CONFIG.NUM_WORKERS}")
        print(f"[INFO] - Initial learning rate: {self.TRAIN_CONFIG.LEARNING_RATE}")
        print(f"[INFO] - Num classes: {self.TRAIN_CONFIG.NUM_CLASSES}")
        print("[INFO] *** VALID CONFIG ***")
        print(f"[INFO] - Batch size: {self.VALID_CONFIG.BATCH_SIZE}")
        print(f"[INFO] - Num workers: {self.VALID_CONFIG.NUM_WORKERS}")
        print(f"[INFO] *** TRAIN VALID RATIO: {self.TRAIN_VALID_RATIO} ***")

        self.train_loader, self.valid_loader = self.create_dataloader()
        print(f"[INFO] *** TRAIN LEN: {len(self.train_loader.dataset)} ***")
        print(f"[INFO] *** VALID LEN: {len(self.valid_loader.dataset)} ***")
        ...
        for epoch in range(1, self.TRAIN_CONFIG.EPOCH + 1):
            ...
            print(f"Validating accuracy: {valid_accuracy}%")
            ...
            self.writer.add_scalar(
                "@Summary/learning_rate", self.optimizer.param_groups[0]["lr"], epoch
            )
            self.writer.flush()
            ...
            if valid_accuracy >= max_valid_accuracy:
                ...
                torch.save(
                    ...
                    os.path.join(
                        self.MODEL_SAVEPATH, f"{self.PRETRAINED_NAME}_best.pth"
                    ),
                )
            ...
            if os.path.exists(
                os.path.join(
                    self.MODEL_SAVEPATH,
                    f"{self.PRETRAINED_NAME}_epoch_{epoch - 10}.pth",
                )
            ):
                os.remove(
                    os.path.join(
            ...
            torch.save(
                self.model.state_dict(),
                os.path.join(
                    self.MODEL_SAVEPATH, f"{self.PRETRAINED_NAME}_epoch_{epoch}.pth"
                ),
            )
            ...
            eta = str(datetime.timedelta(seconds=eta))
            print(
                f"Runing time: Epoch {epoch}: {str(datetime.timedelta(seconds=int(runtime)))} | ETA: {eta}"
            )
    ...
    def average_weights(self, list_weights_path: list, save_path):
        ...
        print(f"Average {len(list_weights_path)} epoch, save at {save_path}")
    ...
    def convert_onnx(self, weights_path):
        ...
        dummy_input = torch.randn(
            4, 3, self.TRAIN_CONFIG.IMAGE_SIZE, self.TRAIN_CONFIG.IMAGE_SIZE
        ).to(self.DEVICE)
        torch.onnx.export(
    ...
    def infer_onnx(self, onnx_model_path, input_imagepath):
        ...
        img_rgb_cv2 = cv2.resize(
            img_rgb_cv2, (224, 224), interpolation=cv2.INTER_LINEAR
        )
...
if __name__ == "__main__":
    ...
    runner = Runner(
        pretrained_name="mobilenetv3_small_050.lamb_in1k",
